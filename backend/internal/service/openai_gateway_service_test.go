...
func TestOpenAIStreamingCorrectsResponsesToolArgumentDeltas(t *testing.T) {
	gin.SetMode(gin.TestMode)
	svc := &OpenAIGatewayService{
	...
	upstreamSSE := strings.Join([]string{
		`data: {"type":"response.output_item.added","output_index":0,"item":{"id":"fc_1","type":"function_call","call_id":"call_1","name":"exec_command","status":"in_progress"}}`,
		"",
		`data: {"type":"response.function_call_arguments.delta","item_id":"fc_1","output_index":0,"delta":"{\"cmd\":"}`,
		"",
		`data: {"type":"response.function_call_arguments.delta","item_id":"fc_1","output_index":0,"delta":"\"gitcommit-m\\\"Updatechargeanimationassets\\\"\"}"}`,
		"",
		`data: {"type":"response.function_call_arguments.done","item_id":"fc_1","output_index":0,"arguments":"{\"cmd\":\"gitcommit-m\\\"Updatechargeanimationassets\\\"\"}"}`,
		"",
		`data: {"type":"response.completed","response":{"usage":{"input_tokens":1,"output_tokens":1},"output":[{"type":"function_call","id":"fc_1","call_id":"call_1","name":"exec_command","arguments":"{\"cmd\":\"gitcommit-m\\\"Updatechargeanimationassets\\\"\"}"}]}}`,
		"",
	}, "\n")
	resp := &http.Response{
		StatusCode: http.StatusOK,
	...
	require.Contains(t, body, `gitcommit-m`)
	require.NotContains(t, body, `"name":"exec_command"`)
	require.NotContains(t, body, `\"cmd\"`)
}

func TestOpenAIStreamingCorrectsResponsesToolArgumentDeltasFromEventName(t *testing.T) {
	gin.SetMode(gin.TestMode)
	svc := &OpenAIGatewayService{
		cfg:           &config.Config{Gateway: config.GatewayConfig{MaxLineSize: defaultMaxLineSize}},
		toolCorrector: NewCodexToolCorrector(),
	...
	upstreamSSE := strings.Join([]string{
		`data: {"type":"response.output_item.added","output_index":0,"item":{"type":"function_call","call_id":"call_1","name":"exec_command","status":"in_progress"}}`,
		"",
		`data: {"type":"response.function_call_arguments.delta","item_id":"call_1","call_id":"call_1","name":"exec_command","output_index":0,"delta":"{\"cmd\":"}`,
		"",
		`data: {"type":"response.function_call_arguments.delta","item_id":"call_1","call_id":"call_1","name":"exec_command","output_index":0,"delta":"\"gitcommit-m\\\"Updatechargeanimationassets\\\"\"}"}`,
		"",
		`data: {"type":"response.function_call_arguments.done","item_id":"call_1","call_id":"call_1","name":"exec_command","output_index":0,"arguments":"{\"cmd\":\"gitcommit-m\\\"Updatechargeanimationassets\\\"\"}"}`,
		"",
		`data: {"type":"response.completed","response":{"usage":{"input_tokens":1,"output_tokens":1},"output":[{"type":"function_call","call_id":"call_1","name":"exec_command","arguments":"{\"cmd\":\"gitcommit-m\\\"Updatechargeanimationassets\\\"\"}"}]}}`,
		"",
	}, "\n")
	resp := &http.Response{
		StatusCode: http.StatusOK,
...
func TestResponsesToolArgumentDeltasUseTrackedCallID(t *testing.T) {
	svc := &OpenAIGatewayService{toolCorrector: NewCodexToolCorrector()}
	namesByItemID := make(map[string]string)
	state := newResponsesFunctionCallArgumentDeltaState()

	trackResponsesToolNameByItemID(
		[]byte(`{"type":"response.output_item.added","item":{"type":"function_call","call_id":"call_1","name":"exec_command"}}`),
		namesByItemID,
	)
	require.Equal(t, "bash", namesByItemID["call_1"])

	first, changed := svc.correctResponsesFunctionCallArgumentsDelta(
		[]byte(`{"type":"response.function_call_arguments.delta","item_id":"call_1","delta":"{\"cmd\":"}`),
		namesByItemID,
		state,
	)
	require.True(t, changed)
	require.Equal(t, "", gjson.GetBytes(first, "delta").String())

	second, changed := svc.correctResponsesFunctionCallArgumentsDelta(
		[]byte(`{"type":"response.function_call_arguments.delta","item_id":"call_1","delta":"\"pwd\"}"}`),
		namesByItemID,
		state,
	)
	require.True(t, changed)
	require.JSONEq(t, `{"command":"pwd"}`, gjson.GetBytes(second, "delta").String())
}
...
func TestHandleSSEToJSON_ReconstructsImageGenerationCallCompleted(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/", nil)
	...
	body := []byte(strings.Join([]string{
		`data: {"type":"response.image_generation_call.completed","id":"ig_123","b64_json":"aGVsbG8=","revised_prompt":"draw a cat","output_format":"webp"}`,
		`data: {"type":"response.completed","response":{"id":"resp_img","model":"gpt-5.4","output":[],"usage":{"input_tokens":7,"output_tokens":9,"output_tokens_details":{"image_tokens":4}}}}`,
		`data: [DONE]`,
	}, "\n"))
	...
	require.Equal(t, "aGVsbG8=", gjson.Get(rec.Body.String(), "output.0.result").String())
	require.Equal(t, "draw a cat", gjson.Get(rec.Body.String(), "output.0.revised_prompt").String())
	require.Equal(t, "webp", gjson.Get(rec.Body.String(), "output.0.output_format").String())
}
