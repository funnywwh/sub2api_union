...
func applyCodexOAuthTransform(reqBody map[string]any, isCodexCLI bool, isCompact bool) codexTransformResult {
	...
	if isCompact {
		normalizedModel = normalizeOpenAICompactUpstreamModel(normalizedModel)
	}
	if normalizedModel != "" {
...
func normalizeOpenAICompactUpstreamModel(model string) string {
	...
	if strings.HasSuffix(normalizedID, "-openai-compact") {
		return trimmed
	}
	if strings.HasPrefix(normalizedID, "gpt-5.1-codex") {
		return trimmed
	}
	return NormalizeOpenAICompactRoutingModel(trimmed)
}
