	...
	if previousResponseID != "" {
		...
		if selection != nil && selection.Account != nil {
			if !s.isAccountTransportCompatible(selection.Account, req.RequiredTransport) ||
				!isOpenAIAccountTypeCompatible(selection.Account, req.RequiredAccountType) ||
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if account == nil ||
		...
		!isOpenAIAccountTypeCompatible(account, req.RequiredAccountType) ||
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		if fresh == nil ||
			...
			!isOpenAIAccountTypeCompatible(fresh, req.RequiredAccountType) ||
...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatible(account *Account, req OpenAIAccountScheduleRequest) bool {
	...
	if !isOpenAIModelSupportedForRequest(account, req.RequestedModel) {
		return false
	}
...
func (s *OpenAIGatewayService) SelectAccountWithScheduler(
	...
	return s.selectAccountWithScheduler(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, "", requireCompact, strings.TrimSpace(firstOptionalString(requiredAccountType...)))
}
		...
		if requiredTransport == OpenAIUpstreamTransportAny || requiredTransport == OpenAIUpstreamTransportHTTPSSE {
			...
			for {
				...
				if selection.Account.SupportsOpenAIImageCapability(requiredImageCapability) &&
					isOpenAIAccountTypeCompatible(selection.Account, requiredAccountType) {
					return selection, decision, nil
				}
				if selection.ReleaseFunc != nil {
					selection.ReleaseFunc()
