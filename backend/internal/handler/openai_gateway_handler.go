...
func resolveOpenAIResponsesRoutingModel(c *gin.Context, requestedModel string) string {
	model := strings.TrimSpace(requestedModel)
	if model == "" {
		return ""
	}
	if service.IsOpenAIResponsesCompactPathForTest(c) {
		if compactModel := service.NormalizeOpenAICompactRoutingModel(model); compactModel != "" {
			return compactModel
		}
	}
	return model
...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	routingModel := resolveOpenAIResponsesRoutingModel(c, reqModel)
	...
	if previousResponseID != "" {
		...
		// third-party OpenAI-compatible passthrough accounts can resolve a recent
		// response id from the local tool-call context cache after account routing.
	}
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	hooks := &service.OpenAIWSIngressHooks{
		...
		AfterTurn: func(turn int, result *service.OpenAIForwardResult, turnErr error) {
			...
			h.gatewayService.ReportOpenAIAccountScheduleResult(account.ID, true, result.FirstTokenMs)
			conversationID := service.ResolveUsageConversationID(c, firstMessage, nil)
			inboundEndpoint := GetInboundEndpoint(c)
			upstreamEndpoint := GetUpstreamEndpoint(c, account.Platform)
