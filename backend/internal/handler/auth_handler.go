...
func NewAuthHandler(cfg *config.Config, authService *service.AuthService, userService *service.UserService, managedNodeService *service.ManagedNodeService, settingService *service.SettingService, promoService *service.PromoService, redeemService *service.RedeemService, totpService *service.TotpService) *AuthHandler {
	return &AuthHandler{
		cfg:                cfg,
...
// ExchangeFederationTicket consumes a one-time federation ticket and returns
// a short-lived admin access token without issuing a refresh token.
// POST /api/v1/auth/federation/exchange
func (h *AuthHandler) ExchangeFederationTicket(c *gin.Context) {
	if h.managedNodeService == nil {
		response.InternalError(c, "Managed node federation unavailable")
		return
	}
	var req FederationExchangeRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	}
	token, expiresIn, redirect, err := h.managedNodeService.ExchangeFederationTicket(c.Request.Context(), req.Ticket)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
