...
export const BILLING_MODE_PER_HOUR = 'per_hour'
...
export function getBillingModeLabel(mode: string | null | undefined, t: (key: string) => string): string {
  switch (mode) {
    ...
    case BILLING_MODE_PER_HOUR: return t('admin.usage.billingModePerHour')
...
export function getBillingModeBadgeClass(mode: string | null | undefined): string {
  switch (mode) {
    ...
    case BILLING_MODE_PER_HOUR: return 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300'
