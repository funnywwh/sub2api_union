...
const chartData = computed(() => {
  ...
  return {
    ...
    datasets: [
      {
        data: displayModelStats.value.map((m) => safeNumber(props.metric === 'actual_cost' ? m.actual_cost : m.total_tokens)),
...
const rankingChartData = computed(() => {
  ...
  const data = props.rankingItems.map((item) => safeNumber(item.actual_cost))
  const backgroundColor = chartColors.slice(0, props.rankingItems.length)

  if (otherRankingItem.value) {
    labels.push(t('admin.dashboard.spendingRankingOther'))
    data.push(safeNumber(otherRankingItem.value.actual_cost))
...
const otherRankingItem = computed<RankingDisplayItem | null>(() => {
  ...
  const rankedActualCost = props.rankingItems.reduce((sum, item) => sum + safeNumber(item.actual_cost), 0)
